...
public class PatrolSystem : IInitializeSystem, IExecuteSystem
{
    ...
    private IGroup<GameEntity> m_GroupReadyToPatrol;
    private IGroup<GameEntity> m_GroupPatroling;
    ...
    public void Initialize()
    {
        m_GroupReadyToPatrol =
            m_Contexts.game.GetGroup(GameMatcher.AllOf(GameMatcher.Patrol).NoneOf(GameMatcher.PatrolPaths,GameMatcher.RequestPath));
        m_GroupPatroling = m_Contexts.game.GetGroup(GameMatcher.AllOf(GameMatcher.Patrol, GameMatcher.PatrolPaths));
    }

    public void Execute()
    {
        foreach (GameEntity entity in m_GroupReadyToPatrol.GetEntities())
        {
            InitPatrolData(entity);
        }
        
        foreach (GameEntity entity in m_GroupPatroling.GetEntities())
        {
            DoPatrol(entity);
        }
    }

    private async void DoPatrol(GameEntity entity)
    {
        ...
        var view = entity.view.value as View;
        // var characterController = view.GetComponent<CharacterController>();
        // if (characterController == null)
        // {
        //     characterController = view.gameObject.AddComponent<CharacterController>();
        // }
    ...
    private void InitPatrolData(GameEntity entity)
    {
        ...
        // obj1.transform.position = targetPoint;
