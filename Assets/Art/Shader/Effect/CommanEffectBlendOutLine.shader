Shader "YanJia/Effect/CommanEffectBlendOutLine"
{
Properties
	{
	...
	Category 
	{
		...
		fixed4 frag (v2f i, float facing : VFACE) : SV_Target
		{
            ...
            col.a = max(0, maskTex.r - _CutOff > 0 ? mainTex.a : 0);
		...
		fixed4 fragAlpha (v2f i) : SV_Target
		{
            ...
            fixed edgeAlpha = lerp(0, _EdgeBloomPower, maskTex.r - _CutOff > _EdgeLength ? 0 : 1);
            alpha = maskTex.r - _CutOff > 0 ? alpha + edgeAlpha : 0;
            alpha *= _Alpha;
		...
		{
			return fixed4(_OutlineColor, _Alpha * i.color.a) ;
		}
