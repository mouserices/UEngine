Shader "YanJia/Effect/CommanEffectBlendRotate"
{
Properties
	{
		...
		[Space(20)]
		_MainTexRotate("MainTex Rotate", Range(-10, 10)) = 0
		_MaskTexRotate("MaskTex Rotate", Range(-10, 10)) = 0
		_AlphaTexRotate("AlphaTex Rotate", Range(-10, 10)) = 0
        [Space(20)]
	...
	Category 
	{
		...
		v2f vert (a2v v)
		{
			...
			o.color = v.color;
			o.uv = TRANSFORM_TEX(v.uv, _MainTex) - float2(0.5, 0.5);
			half mainAngle = _Time.y * _MainTexRotate;
			o.uv = float2(o.uv.x * cos(mainAngle) - o.uv.y * sin(mainAngle), o.uv.x * sin(mainAngle) + o.uv.y * cos(mainAngle));
			o.uv += float2(0.5 ,0.5) + float2(_MainTexX * _Time.y, _MainTexY * _Time.y);
			half maskAngle = _Time.y * _MaskTexRotate;
            o.uvMask = TRANSFORM_TEX(v.uv, _MaskTex) - float2(0.5, 0.5);
			o.uvMask = float2(o.uvMask.x * cos(mainAngle) - o.uvMask.y * sin(mainAngle), o.uvMask.x * sin(mainAngle) + o.uvMask.y * cos(mainAngle));
			o.uvMask +=  float2(0.5, 0.5) + float2(_MaskTexX * _Time.y, _MaskTexY * _Time.y);
			o.uvAlpha = TRANSFORM_TEX(v.uv, _AlphaTex) - float2(0.5, 0.5);
			half alphaAngle = _Time.y * _AlphaTexRotate;
			o.uvAlpha = float2(o.uvAlpha.x * cos(alphaAngle) - o.uvAlpha.y * sin(alphaAngle), o.uvAlpha.x * sin(alphaAngle) + o.uvAlpha.y * cos(alphaAngle));
			o.uvAlpha += float2(0.5,0.5) + float2(_AlphaTexX * _Time.y, _AlphaTexY * _Time.y);
