Shader "YanJia/Charater/CrRoleBlend"
{
    Properties 
	{
        ...
        _SubSpeShaTex ("SubSpeShaTex", 2D) = "white" {}
        ...
        _CharaterCenter("Charater Center", Float) = 0
        _CharaterHeight("Charater Height", Float) = 2
        _BlendIntensity("Blend Iintensity", Range(0,2)) = 2
        _BlendHeight("Blend Height", Range(0.1,0.5)) = 0.3
        _Blend("Charater Tex Blend", Range(0,1)) = 0
    }
    ...
    SubShader 
	{
        ...
        Pass 
		{
			...
			fixed4 _LigDir;
            fixed _BloomPower;
            fixed _CharaterCenter;
            fixed _CharaterHeight;
            ...
            v2f vert (a2v v)
			{
                ...
                //fixed center = _CharaterCenter + _CharaterHeight * _Blend;
                //o.fade = smoothstep(-0.1, 1.1, (center - o.worldPos.y) * 10);
                return o;
            }
            ...
            fixed4 frag(v2f i) : SV_TARGET 
			{
                ...
                fixed center = _CharaterCenter + _CharaterHeight * _Blend;
                fixed fade = smoothstep(0, 1, (center - i.worldPos.y) * (1 / _BlendHeight));
                fixed4 lightTexColor;
                fixed4 baseTexColor;
                if(fade <= 0.3)
                {
                    lightTexColor = tex2D(_SpeShaTex,TRANSFORM_TEX(i.uv, _SpeShaTex));
                    baseTexColor = tex2D(_MainTex,TRANSFORM_TEX(i.uv, _MainTex));
                }
                if (fade > 0.3 && fade < 0.7)
                {
                    lightTexColor = tex2D(_SpeShaTex,TRANSFORM_TEX(i.uv, _SpeShaTex)) * _BlendIntensity;
                    baseTexColor = tex2D(_MainTex,TRANSFORM_TEX(i.uv, _MainTex)) * _BlendIntensity;
                }
                if (fade >= 0.7)
                {
                    lightTexColor = tex2D(_SubSpeShaTex,TRANSFORM_TEX(i.uv, _SpeShaTex));
                    baseTexColor = tex2D(_SubTex,TRANSFORM_TEX(i.uv, _MainTex));
                }
                ...
                //col.rgb = fixed3(i.fade, i.fade, i.fade);
                return col;
            }
            ENDCG
        }
