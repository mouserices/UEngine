Shader "YanJia/Charater/CelCharaterTransparent"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				col.a = max(0, maskTex.r - _CutOff > 0 ? col.a : 0);
		...
		Pass 
		{
            ...
            fixed4 frag(v2f i) : SV_TARGET 
			{
				//溶解消失
				fixed4 maskTex = tex2D(_MaskTex, i.uvMask);
				fixed alpha = max(0, maskTex.r - _CutOff > 0 ? 1 : 0);
        ...
        Pass 
        {
           ...
           struct a2v
			...
			fixed4 frag (v2f i) : SV_Target
			{
				fixed4 col = fixed4 (0,0,0, _BloomPower);
				return col;
			}
			ENDCG
        }
