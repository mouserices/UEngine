...
public class NP_SearchTargetService : NP_BaseService
{
    ...
    public override Action GetServiceAction()
    {
        return new Action(() =>
        {
            ...
            var entity = Contexts.sharedInstance.unit.GetEntityWithUnit(this.Skill.UnitID);
            if (entity.camp.Value == CampType.Enemy)
            {
                campType = CampType.Own;
            }

            var entitiesWithCamp = Contexts.sharedInstance.unit.GetEntitiesWithCamp(campType);
